...
public class TenantMapperImpl implements TenantMapper {
    ...
    public Tenant requestDtoToEntity(TenantRequestDto requestDto) {
        ...
        rent.setMonthNumber(LocalDate.now().getMonthValue()).setDueOfThisMonth(requestDto.getTenantRent()).setPaid(false);
        return new Tenant().setEmail(requestDto.getEmail()).setTenant(true).setName(requestDto.getName()).setPhoneNumber(requestDto.getPhoneNumber()).setAdharNumber(requestDto.getAdharNumber()).setRents(List.of(rent)).setDepositeAmount(requestDto.getDepositeAmount()).setCreateTime(LocalDate.now()).setTotalStayMonth(0).setRentAmount(requestDto.getTenantRent());
    }
